<template>
  <div class="autocomplete-field" @keydown="onKeydown" @keyup="onKeyup">
    ...
    <place-autocomplete-list
      v-if="predictions && showPredictions"
      :items="predictions"
...
export default {
  ...
  watch: {
    value (value) {
      console.log('address on map', value)
      ...
      if (this.query !== value) {
        ...
        this.query = value
        this.$nextTick(async () => {
          this.predictions = await this.search()
        })
      } else {
        this.query = value
      }
  ...
  mounted () {
    ...
    if (this.apiKey) {
      script(
        `${this.baseUri}?key=${this.apiKey}&libraries=${this.libraries.join(
          ','
        )}`
      )
        .then((res) => {
