...
<template>
  <label class="cuisineSearch">
    ...
    <ul class="cuisineSearch_list" :class="{ open: listIsOpen }">
      <li
        v-for="(cuisine, i) in searchableCuisines"
        :key="i"
        class="cuisineSearch-item"
        @click.stop="setValue(cuisine)"
      >
        <p class="cuisineSearch-item-text capitalize">{{ cuisine.name }}</p>
      </li>
    </ul>
  </label>
</template>
...
export default {
  ...
  computed: {
    searchableCuisines () {
      ...
      const tops = this.CATERING_FILTER_SETTINGS.cuisine.mainList.filter(
        (cuisine, i) => {
          if (cuisine.name.search(regexp) !== -1) {
            return true
          }
        }
      )
